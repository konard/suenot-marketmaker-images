#!/usr/bin/env python3
"""Validate the generated image set for issue #69."""
...
MAX_BYTES = 1_500_000
FILES = (
    "uniswap-v3-concentrated-liquidity-quants.png",
    "uniswap-v3-concentrated-liquidity-quants-virtual-reserves.png",
    "uniswap-v3-concentrated-liquidity-quants-tick-grid.png",
    "uniswap-v3-concentrated-liquidity-quants-fee-accounting.png",
    "uniswap-v3-concentrated-liquidity-quants-short-vol-payoff.png",
)
...
def decode_png(path: Path) -> tuple[int, int, int]:
    ...
    while offset < len(data):
        ...
        assert chunk_type not in TEXT_CHUNKS, f"{path}: unexpected text metadata"
    ...
    assert saw_idat and saw_iend, f"{path}: incomplete PNG structure"
    assert decompressor.eof, f"{path}: incomplete compressed image stream"
    assert offset == len(data), f"{path}: data after IEND"

    channels = {2: 3, 6: 4}.get(color_type)
    assert channels is not None, f"{path}: expected RGB/RGBA, got type {color_type}"
    expected_bytes = height * (1 + width * channels)
    assert decompressed_bytes == expected_bytes, (
        f"{path}: decoded {decompressed_bytes:,}, expected {expected_bytes:,} bytes"
    )
...
def main() -> None:
    for filename in FILES:
        ...
        assert MIN_BYTES <= file_size <= MAX_BYTES, (
            f"{filename}: expected {MIN_BYTES:,}-{MAX_BYTES:,} bytes, got {file_size:,}"
        )
        width, height, _ = decode_png(path)
        assert (width, height) == EXPECTED_SIZE, (
            f"{filename}: expected {EXPECTED_SIZE}, got {(width, height)}"
        )
        print(f"ok: {filename} ({file_size:,} bytes, {width}x{height})")
